# StackPicks 記事トピックバックログ

記事の企画・進行管理用ファイル。上から順に執筆予定。

## 執筆待ち

### 1. freee vs マネーフォワード クラウド会計 比較
- カテゴリ: 比較レビュー
- ターゲット: 小規模事業者・フリーランス
- 切り口: 確定申告・経理の自動化。UI/操作性、銀行連携、料金、サポート
- アフィリエイト: freee、マネーフォワード
- 優先度: ★★★（検索ボリューム大・収益性高）

### 2. Canva vs Adobe Express 比較 ── 非デザイナーが選ぶべきツール
- カテゴリ: 比較レビュー
- ターゲット: 中小企業のマーケ担当・SNS運用者
- 切り口: デザイン未経験者が業務で使う視点。テンプレ品質、ブランドキット、チーム共有
- アフィリエイト: Canva Pro、Adobe Express
- 優先度: ★★★

### 3. Zoom vs Google Meet vs Microsoft Teams（ビデオ会議）比較
- カテゴリ: 比較レビュー
- ターゲット: リモートワーク導入中の中小企業
- 切り口: 無料プラン制限、録画・文字起こし、大人数会議、セキュリティ
- アフィリエイト: Zoom
- 優先度: ★★☆

### 4. kintone 導入ガイド ── ノーコードで業務アプリを作る方法
- カテゴリ: 導入ガイド
- ターゲット: 紙・Excel管理から脱却したい中小企業
- 切り口: 具体的な業務アプリの作り方、導入事例、料金、他ツールとの連携
- アフィリエイト: kintone
- 優先度: ★★☆

### 5. Google Workspace vs Microsoft 365 比較
- カテゴリ: 比較レビュー
- ターゲット: グループウェア導入を検討中の中小企業
- 切り口: コスト、管理のしやすさ、セキュリティ、既存環境との相性
- アフィリエイト: Google Workspace、Microsoft 365
- 優先度: ★★★

### 6. HubSpot CRM 無料プラン導入ガイド
- カテゴリ: 導入ガイド
- ターゲット: 顧客管理をExcelでやっている中小企業
- 切り口: 無料でどこまでできるか、営業プロセス改善、MA連携
- アフィリエイト: HubSpot
- 優先度: ★★☆

### 7. AI議事録ツール比較（tl;dv vs Otter vs Notta）
- カテゴリ: AIツール
- ターゲット: 会議が多い中小企業
- 切り口: 日本語精度、Zoom/Meet連携、要約品質、セキュリティ
- アフィリエイト: tl;dv, Notta
- 優先度: ★★☆

### 8. Figma 入門ガイド ── エンジニアでなくても使えるデザインツール
- カテゴリ: 導入ガイド
- ターゲット: 非デザイナーのPM・マーケター
- 切り口: ワイヤーフレーム作成、プロトタイプ共有、デザイナーとの連携
- アフィリエイト: Figma
- 優先度: ★☆☆

### 9. Lark vs Notion vs Coda ── オールインワンツール比較
- カテゴリ: 比較レビュー
- ターゲット: ツールを統合したい中小企業
- 切り口: ドキュメント＋プロジェクト管理＋コミュニケーションの統合度
- アフィリエイト: Notion, Lark
- 優先度: ★☆☆

### 10. 中小企業向けセキュリティツール入門（1Password, NordVPN, etc.）
- カテゴリ: 導入ガイド
- ターゲット: セキュリティ対策が不十分な中小企業
- 切り口: パスワード管理、VPN、エンドポイント保護の3本柱
- アフィリエイト: 1Password, NordVPN
- 優先度: ★★☆

## 公開済み
- [x] Notion vs Obsidian 徹底比較（2026-04-07）
- [x] AIライティングツール5選（2026-04-07）
- [x] Shopify vs BASE vs STORES 比較（2026-04-07）
- [x] Slack vs Chatwork vs Teams 比較（2026-04-07）
- [x] Make ノーコード自動化入門ガイド（2026-04-07）
